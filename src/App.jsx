...
function App() {
  ...
  const getProcessedData = () => {
    ...
    matchedGroups.forEach(g => {
      if (g.children) g.children.forEach(c => matchedMappedCodes.add(c.code));
    });
    ...
    return expandedResult.map(item => {
      const cost = Number(item.cost || 0); const tag = Number(item.tag_price || 0);
      const sale = Number(item.price_sale || 0); 
      const fee = Math.floor(sale * 0.18); const settle = sale - fee; const margin = settle - cost - 5000;
  ...
  const saveEdit = async (item) => {
    ...
    if (!error) { alert("수정 완료"); setEditingCode(null); fetchData(); }
  };
  ...
  const downloadListExcel = () => {
    ...
    if (selectedCodes.length > 0) sourceData = sourceData.filter(item => selectedCodes.includes(item.code));
    ...
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "조회데이터");
  ...
  const compareInputStyle = { width: '50px', fontSize: '10px', padding: '0px', border: '1px solid #3498db', borderRadius: '2px' };
  const batchInputStyle = { width: '50px', fontSize: '10px', padding: '2px', border: '1px solid #ccc', borderRadius: '3px' };
      ...
      <div style={{ flex: 1, padding: '20px', boxSizing: 'border-box', marginLeft: '85px', width: 'calc(100% - 85px)', overflowY: 'auto' }}>
        ...
        {activeMenu === 'list' && (
          <div style={{ width: '100%' }}>
            ...
            <div style={{ background:'#ebf3f9', padding:'6px 12px', borderRadius:'8px', marginBottom:'15px', display:'flex', gap:'10px', alignItems:'center', border:'1px solid #3498db' }}>
                  ...
                  {getProcessedData().map((item, idx) => {
                    ...
                    return (
                      <tr key={`${item.code}-${idx}`} style={{ background: trBg }}>
                        ...
                        <td style={tdStyle}>{isGhost ? <span style={{color:GHOST_COLOR}}>-</span> : (<>{(item.price_coupang || 0).toLocaleString()} → {isE ? <input type="number" value={editRow.price_coupang} onChange={e=>setEditRow({...editRow, price_coupang:e.target.value})} style={compareInputStyle}/> : "-"}</>)}</td>
                        <td style={tdStyle}>{isGhost ? <span style={{color:GHOST_COLOR}}>-</span> : (<>{(item.price_rocket || 0).toLocaleString()} → {isE ? <input type="number" value={editRow.price_rocket} onChange={e=>setEditRow({...editRow, price_rocket:e.target.value})} style={compareInputStyle}/> : "-"}</>)}</td>
                        <td style={tdStyle}>{isGhost ? <span style={{color:GHOST_COLOR}}>-</span> : (<>{(item.price_gold || 0).toLocaleString()} → {isE ? <input type="number" value={editRow.price_gold} onChange={e=>setEditRow({...editRow, price_gold:e.target.value})} style={compareInputStyle}/> : "-"}</>)}</td>
